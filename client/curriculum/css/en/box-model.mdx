export const CONFIG = {
  chapters: [
      { title: 'Components of the Box Model', id: 'componentele-box-model' },
      ...
      { title: 'Interactive Box Model', id: 'box-model-interactiv' },
    ],
};
...
{<LessonFirstSentence> So far, we've learned what CSS means, what we can use it for, and, very importantly, how to use it. CSS is a language that allows us to create websites with excellent designs. At its core, each element is just a rectangle with a few special properties. </LessonFirstSentence>}

In this lesson, we will learn about the box model: what it is composed of and how it is used "behind the scenes" to create those WOW designs. Understanding how it works is the key that will help you build any interface and solve various layout problems that may arise.
...
<section>
  ...
  <strong>The Box Model</strong> is composed of 4 properties:
  
  {<List variant="bullets">
    <li> <strong>Content</strong>: The "box" content, where text/image/etc. is displayed.</li>
    <li> <strong>Padding</strong>: A transparent space that surrounds the content.</li>
    <li> <strong>Border</strong>: An "outline" that surrounds the content and padding.</li>
    <li> <strong>Margin</strong>: A transparent area outside the border that helps to space elements apart.</li>
  </List>}
    
  <LessonFigure
    withBorder
    src={`${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/box-model-intro.png`}
    alt="Box Model visualized - MDN"
  />

  Normally, if we don't intervene, most elements would be stuck together. However, browsers apply some margins to certain elements, especially heading tags (<FormattedText as="strong">{'<h1>'}</FormattedText> - <FormattedText as="strong">{'<h6>'}</FormattedText>) and paragraphs (<FormattedText as="strong">{'<p>'}</FormattedText>), but we won't go into detail now.
...
<div>Element</div>`} />
  ...
  <LessonFigure
    ...
    alt="Two elements without spacing"
  />

  As can be seen, both <FormattedText as="strong">{'<div>'}</FormattedText> tags are stuck together. We don't have a defined spacing, nor any borders to separate them. Let's see how we can use the components of the box model to change the way the two <FormattedText as="strong">{'<div>'}</FormattedText> elements are displayed.
</section>
<section>
  ...
  We'll start with the border. Border won't actually add spacing between elements, but it will help set some visual limits for them.
...
<LessonFigure
  ...
  alt="Two elements with borders"
/>

  Setting a border for the two divs helps a little with separation, but the elements are still stuck together. It's far from that WOW design which we want. To separate them a little, we can use <strong>margin</strong>.
</section>
<section>
  ...
  <LessonFigure
    ...
    alt="Two elements with margin"
  />

  We can already see an improvement. The two divs are no longer glued together, but have some space between them, exactly 12px.

  {<LessonTip> Although we set 12px as the margin for both <FormattedText as="strong">{'<div>'}</FormattedText>s, the distance between the two elements will not be 24px (12px * 2), but will remain 12px. This happens because of a concept called <strong>margin collapsing</strong>.

  It's a phenomenon that deserves its own mini-lesson, but what you need to know for now is that sometimes two margins can collapse into a single margin, which will have the maximum value between the two individual margins. You can read more <Link prefetch={false} target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Box_Model/Mastering_margin_collapsing">here</Link>. </LessonTip>}

  If we open DevTools and inspect one of the <FormattedText as="strong">{'<div>'}</FormattedText>s, we will see the image below. Specifically, the orange border represents the margin we set in the code, 12px in all directions (top, right, bottom, left).

  <LessonFigure
    withBorder
    src={`${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/devtools-margin.png`}
    alt="DevTools opened in inspect mode"
  />
</section>
<section>
  ...
  <strong>Padding</strong> is used to add space between content and border. In our example, the padding would be displayed between the word "Element" and the 1px black border we set earlier.
  ...
  <LessonFigure
    ...
    alt="Element with padding applied"
  />

  The green box represents the equivalent of the 8px padding that we set. As you can see, the word "Element" is now further away from the border.
</section>
<section>
  ...
  Okay, now that we've learned about border, margin, and padding, it's time to see how they all apply to the example element. For this, we'll use <strong>DevTools</strong> again.

  <LessonFigure
    withBorder
    src={`${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/devtools-inspect-element.png`}
    alt="Element with all box model components visible"
  />

  <LessonFigure
    withBorder
    src={`${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/devtools-full.png`}
    alt="The same element viewed in DevTools"
  />

  The first image should be familiar to you. There, you'll find the visually enhanced element with a graphical representation of all the properties that make up the box model.

  In the second image, you'll see the exact dimensions set for margin, border, and padding. The blue box corresponds to the <strong>content</strong>, which in this example has a width of <em>372px</em> and a height of <em>19px</em>.

  Let's set the <FormattedText as="strong">width</FormattedText> and <FormattedText as="strong">height</FormattedText> properties as well, to see them change in the graphical representation.
  ...
  <LessonFigure
    ...
    alt="Complete box model, viewed in DevTools"
  /> 
    
  Okay, now we've set all the values of a box model. We can see this on the element in the page as well.
        
  <LessonFigure
    withBorder
    src={`${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/rendered-element.png`}
    alt="Complete box model, viewed on the page"
  />

  Hmmm, something's not right. We set the width to <em>100px</em> and the height to <em>50px</em>, but our div has a width of <em>118px</em> and a height of <em>68px</em>.
...
<section>
  ...
  To understand what happened, we need to talk a little about the <FormattedText as="strong">box-sizing</FormattedText> property. This property controls how the total size of an element (width and height) is calculated.

  In CSS, if we set a width of 100px, it will be applied to the <strong>content</strong>. When we add padding or border, these values will be added to the total size of the final element that will be rendered on the page.

  Let's go back to our example. We will focus on the width, as the height is calculated in the same way.
  ...
  We have set a width of 100px, a padding of 8px, and a border of 1px. The way the width is calculated is as follows:

  <LessonQuote>
    100px + 8px * 2 + 1px * 2 = 118px
  </LessonQuote>

  {<LessonTip> Why 8px * 2? We need to take into account the <FormattedText as="strong">padding-left</FormattedText> and <FormattedText as="strong">padding-right</FormattedText>. The same applies to the <FormattedText as="strong">border</FormattedText>. </LessonTip>}

  As you can see, it's not necessarily an easy way to set dimensions. If we need a certain width, we have to think about what border and padding we will set to achieve the desired result. That doesn't sound too good, does it?

  Fortunately, CSS provides us with a solution: the <FormattedText as="strong">box-sizing</FormattedText> property. By default, this property has a value of <FormattedText as="strong">content-box</FormattedText>, which means that the <em>width</em> and <em>height</em> will be applied to the content.

  The other value is <FormattedText as="strong">border-box</FormattedText>. Setting this value will tell the browser to take into account any added border or padding when calculating dimensions. The content will be shrunk to be able to reach the set width or height.

  In the images below, we can see these differences exactly at the content level.

  <SideBySidePictures
    direction="row"
    img1={{
      src: `${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/devtools-box-sizing-content-box.png`,
      alt: 'Element with box-sizing: content-box, viewed from DevTools',
    }}
    img2={{
      src: `${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/devtools-box-sizing-border-box.png`,
      alt: 'Element with box-sizing: border-box, viewed from DevTools',
    }}
  />
  
  The difference is also visible on the page, where the elements are rendered in their final form, after all calculations have been made.

  <LessonFigure
    withBorder
    src={`${SCOPE.CLOUDFRONT_PUBLIC}/public/images/lessons/box-model/rendered-elements-final.png`}
    alt="Top -> border-box, bottom -> content-box"
  />
</section>
<section>
  ...
  Alright, enough talk. Let's move on to something practical.

  Below, we've recreated the box model from DevTools so that you can interact with it and see in real-time how the size of the element is affected by the <FormattedText as="strong">box-sizing</FormattedText> property.

  {<LessonTip> The default values set on the left serve as maximum limits to avoid breaking the general layout of the page. To interact with the values, simply set any positive value smaller than this maximum. When you change the active checkbox, you should see the final value change based on your choice. </LessonTip>}

  <DevToolsClone />
</section>
<div className="dots" />
<section>
  We've covered quite a bit of theory in this lesson. Hopefully, it's a little clearer now how the box model is used in CSS. If you still feel like you need more information, you can use one of the resources below:
</section>
