      ...
      />
       <Lesson title="Moduri de stilizare" chapters={chapters}>
        ...
        <section>
          ...
          <p>Prin <strong>metoda inline</strong> putem aplica stilul unui singur element HTML.</p>
          <p>Scriere codului CSS se va face prin adaugarea atributului 
          {' '}
          ...
          <LessonFigure
            withBorder
            src="/images/lessons/git waysdesign/inline.png"
            alt="Folosirea metodei inline pentru aplicarea stilului."
            demo="/demo/html/metoda-inline"
          />
          <p>
          Metoda inline vine la pachet cu câteva <strong>dezavantaje uriașe</strong> și din această cauză majoritatea programatorilor o evită. Câteva dintre acestea sunt:
          </p>
          <ul className="with--bullets">
            <li className="mb-4">
              ...
              <strong className="formatted">{'DRY'}</strong>{' '} (<strong>Don’t repeat yourself</strong>).
            </li>
            <li>
            Vedem că e greu să distingem codul CSS de cel HTML (se produce un așa numit efect de <strong>spaghetti code</strong>).
          ...
          <p>Dacă prin metoda inline putem stiliza câte un element pe rând, printr-un <strong>fișier intern</strong> putem controla
             stilul pentru o întreagă pagină, scăpând astfel de codul duplicat și, totodată, 
             vom avea o mai bună înțelegere a codului și vom eficientiza timpul de scriere.</p>
          <p>Metoda de integrare a codului CSS în pagina web se va face prin inserarea elementului
          {' '}
            <strong className="formatted">{'<style>'}</strong> 
          {' '}
            în secțiunea
          ...
          <Highlight
            ...
            `}
          />
          <p>În acest exemplu, <strong>selectorul</strong> folosit este {' '}
            <strong className="formatted">{'p'}</strong> 
            , reprezentând tag-ul elementului din HTML.</p>
          <LessonFigure
            withBorder
          ...
          <p>Care ar fi soluția pentru a scapa de dezavantajele metodelor anterioare? Metoda <strong>fișierului extern</strong> all the way!</p>
          <p>Aceasta este metoda pe care o recomandăm, 
            și constă în crearea unui fișier separat pe care-l asociem paginilor HTML.</p>
          <p>Integrarea fișierului CSS în HTML se va face prin intermediul elementului {' '}<strong className="formatted">{'<link>'}</strong>.</p>
          ...
          <p>Dacă creăm fișierul 
            ...
            <strong className="formatted">{'index.html'}</strong>
             , head-ul filei HTML va arăta astfel:</p>
             <Highlight
            className="my-5"
            language="html"
            code={`
          ...
          <p>Poate v-ați întrebat de ce punem {' '}
          <strong className="formatted">{'<link>'}</strong>
          {' '} 
          in HEAD și nu în BODY?</p>
          <p>Explicația ar fi că browser-ul citește documentul HTML <strong>de sus în jos</strong>. 
            Astfel, primul pe care îl vede este <strong>HEAD-ul</strong>. </p>
            <p>Având elementul 
            {' '}
          <strong className="formatted">{'<link>'}</strong>
          {' '}  in {' '}
          <strong className="formatted">{'<head>'}</strong>, 
            ...
            ceea ce nu vrem să se întâmple.</p>
          </section>
          </Lesson>
    </>
  );
