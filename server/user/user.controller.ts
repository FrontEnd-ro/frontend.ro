...
import { MAX_DESCRIPTION_LENGTH, MAX_NAME_LENGTH, ServerError,setTokenCookie } from "server/ServerUtils"
...
import SubscriberModel from "server/subscribe.model"
...
class UserController{
  ...
  static async updateName(req:Request,res:Response) {
    ...
    try {
      const updatedUser = await UserController.updateUserFields({ _id: user._id, username: user.username, password }, { name });
  ...
  static async updateUserFields({ _id, username, password }, fields) {
  ...
  static async updateDescription(req:Request,res:Response) {
    ...
    if (description.length > MAX_DESCRIPTION_LENGTH) {
      new ServerError(
        400,
        `Descrierea poate avea maxim ${MAX_DESCRIPTION_LENGTH} caractere.`
      ).send(res);
      return;
    }

    try {
      const updatedUser = await UserController.updateUserFields(
        { _id: user._id, username: user.username, password },
        { description }
      );
      res.json(UserModel.sanitize(updatedUser));
    } catch (err) {
  ...
  static uploadAvatar(req:Request,res:Response) {
    ...
    upload.single("file")(req, null, async (err) => {
      ...
      if (req.file.size > MAX_MEDIA_BYTES) {
        ...
        new ServerError(
          400,
          `Dimensiunea maximă a unui fișier este de ${MAX_MEDIA_MB}MB`
        ).send(res);
        return;
      }
      ...
      try {
        ...
        new ServerError(
          500,
          err.message ||
            "Oops! Se pare că nu am putut încărca fișierele. Încearcă din nou."
        ).send(res);
      }
    });
  }
  ...
  static async subscribe(req:Request,res:Response) {
    ...
    const alreadySubscribed = await SubscriberModel.exists(email);
    if (alreadySubscribed) {
      new ServerError(400, "Hmm, încerci să te abonezi încă o data...🤔").send(
        res
      );
      return;
    }
  
    await SubscriberModel.subscribe({ name, email });
   ...
   static async  deleteAccount(req:Request,res:Response) {
    ...
    if (!areCredentialsOk) {
      new ServerError(
        403,
        "Credențialele introduse nu se potrivesc cu contul tău"
      ).send(res);
      return;
    }
